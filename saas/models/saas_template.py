...
class SAASTemplateLine(models.Model):
    ...
    def create_db(self, key_values=None, db_name=None, with_delay=True, draft_build_id=None):
        ...
        if with_delay:
            job_create = build.delayable().create_db(self.operator_db_name, self.template_id.template_demo)
            job_install_modules = build.delayable().action_install_missing_mandatory_modules()
            job_build_post_init = self.operator_id.delayable().build_post_init(build, self.template_id.build_post_init, key_values)
